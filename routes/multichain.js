...
/*******************************************************************/
/*******************************************************************/
router.get('/leer_registros/:usuario_id/:desde/:hasta', function(req, res, next) {
    obtenercartera(req.params.usuario_id, function(cartera) {
        options.body = JSON.stringify({
            "method": "liststreampublisheritems",
        ...
        request(options, (error, response, body) => {
            if (error) {
                ...
                for (key in respuesta) {
                  console.log(respuesta[key].time );
                  if (respuesta[key].time >=req.params.desde && respuesta[key].time <=req.params.hasta){

                    fecha = new Date(respuesta[key].time * 1000);
                    retorno[key] = {
                        "id": respuesta[key].key,
                        "datos": JSON.parse(Buffer.from(respuesta[key].data, "hex")),
                        "fecha": fecha.toLocaleString(undefined, {
                            day: 'numeric',
                            month: 'numeric',
                            year: 'numeric',
                            hour: '2-digit',
